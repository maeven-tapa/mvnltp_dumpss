...
// HC-05 uses hardware Serial on Arduino R4 Minima
// Connect HC-05 TX to pin 0 (RX)
// Connect HC-05 RX to pin 1 (TX) through voltage divider
...
void setup() {
  // Initialize Bluetooth using hardware Serial
  ...
  Serial.println("Robot Ready - Send 'S' to start, 'X' to stop");
}
...
void handleBluetooth() {
  if (Serial.available()) {
    ...
    if (command == 'S' && !robotRunning && !isStoppingState) {
      robotRunning = true;
      updateDisplay("RUNNING");
      Serial.println("Robot STARTED");
    }
    // Handle STOP command
    else if (command == 'X' && robotRunning) {
      robotRunning = false;
      stopMotors();
      isStoppingState = true;
      stoppingStartTime = millis();
