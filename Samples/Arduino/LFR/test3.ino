...
void followLine() {
  ...
  // Line following logic with improved lost line recovery
  if (s3 && !s1 && !s5) {
    // Center sensor on line (left and right far sensors off) - go straight
    moveForward(BASE_SPEED);
    lastDirection = 0;
  }
  else if (s2 && !s4 && !s5) {
    // Left-center sensor on line - slight left turn
    turnLeft(TURN_SPEED, BASE_SPEED);
    lastDirection = -1;
  }
  else if (s4 && !s2 && !s1) {
    // Right-center sensor on line - slight right turn
    turnRight(BASE_SPEED, TURN_SPEED);
    lastDirection = 1;
  }
  else if (s1 && !s5) {
    // Far left sensor on line - hard left turn
    turnLeft(100, BASE_SPEED);
    lastDirection = -1;
  }
  else if (s5 && !s1) {
    // Far right sensor on line - hard right turn
    turnRight(BASE_SPEED, 100);
    lastDirection = 1;
  }
  else if (s3) {
    // Center sensor on line (any other combination) - go straight
    moveForward(BASE_SPEED);
    lastDirection = 0;
  }
  else {
    ...
    if (lastDirection == -1) {
      // Was turning left, rotate left to find line
      turnLeft(0, TURN_SPEED);
    }
    else if (lastDirection == 1) {
      // Was turning right, rotate right to find line
      turnRight(TURN_SPEED, 0);
    }
    else {
      // No previous direction, try rotating right
      turnRight(TURN_SPEED, 0);
      lastDirection = 1;
    }
  }
