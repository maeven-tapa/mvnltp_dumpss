...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($stock < $qty) {
            ...
            /* INSERT into NEW orders schema */
            $ins = $pdo->prepare(
                "INSERT INTO orders 
                    (order_code, user_id, customer_name, customer_contact, item_code, quantity, total, status)
                 VALUES
                    (?, ?, ?, ?, ?, ?, ?, 'reserved')"
            );
...
<div class="container">
  ...
  <?php if ($alert): ?>
    <?php if (strpos($alert, 'success|') === 0): ?>
      <div class="alert success"><?= e(substr($alert, 8)) ?></div>
    <?php else: ?>
      <div class="alert error"><?= e($alert) ?></div>
    <?php endif; ?>
  <?php endif; ?>
  ...
  <div class="order-layout">
    ...
    <div class="card" style="position: sticky; top: 100px;">
      ...
      <form method="POST">
        <input type="hidden" name="item_code" value="<?= e($item_code) ?>">
...
<script src="../../assets/js/pow-animation.js"></script>
